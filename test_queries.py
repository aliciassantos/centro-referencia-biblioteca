...
try:
    ...
    instruction = db.scalars(
        select(Instrucao).where(Instrucao.nivel_acesso_id == 2)
    ).all()

    for i in instruction:
        print(
            f"Instrução: {i.titulo} | Visibilidade: privada (ID = {i.nivel_acesso_id})"
        )
    ...
    search = db.scalars(
        select(Instrucao).where(
            (Instrucao.titulo.like("%login%")) | (Instrucao.conteudo.like("%login%"))
        )
    ).all()
    ...
    user_list = db.execute(
        select(Usuario.login, Usuario.id, func.count(Instrucao.id))
        .join(
            Instrucao, Usuario.id == Instrucao.usuario_id, isouter=True
        )  # Mostra quem não tem instruções também
        .group_by(Usuario.id, Usuario.login)
    ).all()

    for login, id, n_instructions in user_list:
        print(
            f"Usuário: {login} | ID: {id} | Quantidade de instruções: {n_instructions}"
        )

from sqlalchemy import Column, Integer, String, Boolean, ForeignKey, DateTime, create_engine, select
...
class Usuario(Base):
    ...
    nivel_acesso_id: Mapped[int] = mapped_column(ForeignKey("nivel_acesso.id"), nullable=False, default=2) 
    ativo: Mapped[bool] = mapped_column(nullable=False, default=True)
    data_criacao: Mapped[datetime] = mapped_column(server_default=func.now(), nullable=False)
    ...
    instrucoes_atualizadas: Mapped[list["Instrucao"]] = relationship(back_populates="atualizador", foreign_keys="Instrucao.usuario_atualizou_id")
    # Entre um usuário que criou uma ou mais instruções (1:N)
    instrucoes_criadas: Mapped[list["Instrucao"]] = relationship(back_populates="criador", foreign_keys="Instrucao.usuario_id")
...
class Instrucao(Base):
    ...
    data_atualizacao: Mapped[datetime] = mapped_column(server_default=func.current_timestamp(), onupdate=func.now(), nullable=False)
    data_criacao: Mapped[datetime] = mapped_column(server_default=func.current_timestamp(), nullable=False)
    ...
    instrucoes_midias: Mapped[list["InstrucaoMidia"]] = relationship(
        back_populates="instrucoes_midias"
    )
'''
...
class InstrucaoTopico(Base):
    ...
    instrucao_id: Mapped[int] = mapped_column(
        ForeignKey("instrucao.id"),
        primary_key=True
    )
    topico_id: Mapped[int] = mapped_column(
        ForeignKey("topico.id"), 
        primary_key=True
    )
    ...
    instrucao: Mapped[Instrucao] = relationship(
        back_populates="instrucao_topico", 
        foreign_keys=[instrucao_id]
    )
    topico: Mapped[Topico] = relationship(
        back_populates="instrucao_topico",
        foreign_keys=[topico_id]
    )
...
class Midia(Base):
    ...
    legenda:  Mapped[str | None] = mapped_column()
